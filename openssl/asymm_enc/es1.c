...
int main(int argc, char** argv) {
    ...
    */
    ...
    unsigned char verify[EVP_PKEY_size(private_key)];
    int ver_len = 0;
    FILE* f_v;
    if ((f_v = fopen(argv[3], "r")) == NULL) {
        fprintf(stderr, "Couldn't open the signature file, try again\n");
        exit(1);
    }
    while ((fread(verify, 1, MAXBUFFER, f_v)) > 0) {}
    // if (ver_len != EVP_PKEY_size(private_key)) {
    //     fprintf(stderr, "Key too short\n");
    //     exit(2);
    // }


    if (CRYPTO_memcmp(signature, verify, sig_len) == 0)
